...
export function renderEmpty(container, message) {
  ...
  // Book a Call button (opens booking modal)
  const calBtn = document.createElement('button');
  calBtn.type = 'button';
  calBtn.className = 'magnetic inline-flex items-center gap-2 px-6 py-3 min-h-[44px] min-w-[44px] bg-primary text-white text-sm font-semibold rounded-full hover:bg-primary-light transition-colors duration-200 shadow-warm';
  calBtn.setAttribute('aria-label', 'Book a call with Yasmin');
  calBtn.innerHTML = `<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"/></svg> Book a Call`;
  calBtn.addEventListener('click', async function () {
    // Use window.BookingModal synchronously if already available
    if (window.BookingModal && window.BookingModal.open) {
      window.BookingModal.open(calBtn);
      return;
    }
    const { openBookingModal } = await import('./booking-modal.js');
    openBookingModal(calBtn);
  });

  ctaContainer.appendChild(whatsAppLink);
  ctaContainer.appendChild(calBtn);
...
export function renderJobDetail(job, container) {
  ...
  if (job.tallyFormId && job.tallyFormId.trim() !== '') {
    ...
    // Email button (filled style) — no Book a Call when Tally form is present
